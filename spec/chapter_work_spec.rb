RSpec.describe NovelTrans::ChapterWork do
  ...
  it "rejects an unknown status without writing a ledger file" do
    expect do
      described_class.new(book_id:, chapter_id: chap_a, status: "done")
  ...
  it "writes one JSON file per chapter id" do
    described_class.new(book_id:, chapter_id: chap_a).save
    described_class.new(book_id:, chapter_id: chap_b).save
  ...
  it "lists saved rows for a book" do
    described_class.new(book_id:, chapter_id: chap_a, status: "fetched").save
    described_class.new(book_id:, chapter_id: chap_b, status: "pending").save
